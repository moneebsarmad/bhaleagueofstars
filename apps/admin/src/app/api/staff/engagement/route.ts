...
export async function GET(request: Request) {
  ...
  const calendarDates = await fetchCalendarDates(supabase, startDate, endDate)
  ...
  entries.forEach((entry) => {
    ...
    const houseName = canonicalHouseName(String(entry.house ?? '').trim())
    if (houseName) {
  ...
  if (baselineRange) {
    ...
    const baselineCalendarDates = await fetchCalendarDates(supabase, baselineRange.start, baselineRange.end)
